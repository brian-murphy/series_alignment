...
def align(csv_selector1, csv_selector2):
    ...
    scaler = Scaler(axes, MOVING_KEY, SCALING_KEY)
...
class Scaler:
    def __init__(self, axes, moving_key, scaling_key):
        ...
        self.data_size = np.size(self.ydata) - np.count_nonzero(np.isnan(self.ydata))
    ...
    def make_scaled_array(self, factor):
        ...
        current_data = self.ydata[self.data_start_index:self.data_start_index+self.data_size]
        ...
        new_y[self.data_start_index:self.data_start_index + np.size(new_data)] = new_data
        return new_y
    ...
    def make_shifted_array(self, dx):
        ...
        new_y[self.data_start_index + dx:end_index + dx] = self.ydata[self.data_start_index:end_index]
        return new_y
